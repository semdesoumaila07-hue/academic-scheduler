...
def _find_user_for_teacher(teacher_id: int):
    """Trouve le UserModel lié a un TeacherModel via teacher_id."""
    try:
        session = db_manager.get_session()
        from sqlalchemy import text
        result = session.execute(
            text("SELECT id FROM users WHERE teacher_id = :tid"),
            {'tid': teacher_id}
        ).fetchone()
        return result[0] if result else None
    except Exception as e:
        print(f"[notification_service] find_user error: {e}")
        return None


def notify_leave_approved(leave_request_id: int, teacher_id: int, teacher_name: str):
    """Notifie l'enseignant que son congé a été approuvé."""
    user_id = _find_user_for_teacher(teacher_id)
    if not user_id:
        print(f"[notification_service] Aucun user lie a teacher_id={teacher_id}")
        return
    create_notification(
        user_id=user_id,
    ...
    print(f"[notification_service] Notification approuve envoyee a user_id={user_id}")


def notify_leave_rejected(leave_request_id: int, teacher_id: int, reason: str = ""):
    ...
    print(f"[notification_service] Notification refus envoyee a user_id={user_id}")
    ...
    try:
        ...
        activities = session.execute(text("""
            SELECT a.id, a.name, a.volume_hours,
                   COALESCE(SUM(
                       (strftime('%H', s.end_time) * 60 + strftime('%M', s.end_time) -
                        strftime('%H', s.start_time) * 60 - strftime('%M', s.start_time)) / 60.0
                   ), 0) as scheduled_hours
            FROM academic_activities a
